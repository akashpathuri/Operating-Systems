...
int dir_add(struct inode dir_inode, uint16_t f_ino, const char *fname, size_t name_len) {
	...
	bio_read(super_block->d_start_blk, data_bitmap);
	...
	// Allocate a new data block for this directory if it does not exist
		...
		bitmap_t inode_bitmap=malloc(BLOCK_SIZE);
		//update the bitmap
		bio_read(super_block->i_start_blk, inode_bitmap);
		unset_bitmap(inode_bitmap,f_ino);
		bio_write(super_block->i_start_blk, inode_bitmap);
		free(inode_bitmap);
		free(data_bitmap);
		free(data_block);
		free(dir_inode_data);
	...
	if(num_valid_entries==0){
		...
		for(i=0;i<16;i++){
			...
			if(dir_inode.direct_ptr[i]==data_block_num){
				...
				bio_write(super_block->d_start_blk,data_bitmap);
				free(data_bitmap);
				break;
			}
		}
...
int rufs_mkfs() {
	...
	//Open the diskfile
	disk_fd=dev_open(diskfile_path);
...
void initialize_file_inode(struct inode* inode){
	...
	inode->type=rufs_FILE;				/* type of the file */
...
void initialize_dir_inode(struct inode* inode){
	...
	inode->valid=1;				/* validity of the inode */
	inode->size=0; //TODO: change to size			/* size of the file */
	inode->type=rufs_DIRECTORY;				/* type of the file */
...
static int rufs_readdir(const char *path, void *buffer, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi) {
	...
	for(dir_ptr_index=0;dir_ptr_index<16;dir_ptr_index++){
		...
		bio_read(super_block->d_start_blk+inode->direct_ptr[dir_ptr_index],data_block);
		for(dir_entry_index=0;dir_entry_index<dirent_per_block;dir_entry_index++){
...
static int rufs_write(const char *path, const char *buffer, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	for(indirect_ptr_index=0;indirect_ptr_index<8;indirect_ptr_index++){
		if(inode->indirect_ptr[indirect_ptr_index]==-1){ // then unallocated
			...
			initialize_direct_ptr_block(super_block->d_start_blk+inode->indirect_ptr[indirect_ptr_index]);// initialize all direct ptr entries to -1 for now
		}
			//read the entire block pointed to by indirect pointer
			bio_read(super_block->d_start_blk+inode->indirect_ptr[indirect_ptr_index],indirect_data_block);
...
static int rufs_unlink(const char *path) {
	...
	for(indirect_ptr_index=0;indirect_ptr_index<8;indirect_ptr_index++){
		...
		if(target_inode->indirect_ptr[indirect_ptr_index]!=-1){
			bio_read(super_block->d_start_blk+target_inode->indirect_ptr[indirect_ptr_index],direct_ptr_block);
			...
			bio_write(super_block->d_start_blk+target_inode->indirect_ptr[indirect_ptr_index],direct_ptr_block);
